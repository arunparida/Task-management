...
const updateTask = async (req, res) => {
  try {
    ...
    return res.status(200).json(new ApiResponse(200, { updatedTask: task }, "Task updated successfully"));
...
const assignTask = async (req, res) => {
  try {
    ...
    if (!assigningUser) {
      return res.status(404).json(new ApiResponse(404, null, "Assigning user not found."));
    }

    // Check if the user is a manager or admin
    if (!['manager', 'admin'].includes(assigningUser.role)) {
      return res.status(403).json(new ApiResponse(403, null, "Only managers and admins can assign tasks."));
    }

    // Find the user to assign the task to
    const assignee = await UserModel.findById(assignedTo);
    if (!assignee) {
      return res.status(404).json(new ApiResponse(404, null, "User to assign task to not found."));
    }

    // If the assigning user is a manager, ensure the assignee is in their team
    if (assigningUser.role === 'manager' && assignee.managerId.toString() !== assigningUser._id.toString()) {
      return res.status(403).json(new ApiResponse(403, null, "Managers can only assign tasks to users in their team."));
    }
    ...
    res.status(200).json(new ApiResponse(200, { task: task }, "Task assigned successfully"));
